...
def responseState(func, BaseResponse):
    ...
    if DEBUG or Ret != 0:
        print('func: %s, Ret: %d, ErrMsg: %s' % (func, Ret, ErrMsg.encode('utf-8')))
...
#    print(data)
    ...
    for i in range(len(MemberList) - 1, -1, -1):
        ...
        if Member['VerifyFlag'] & 8 != 0:  # 公众号/服务号
            ...
            MemberList.remove(Member)
        elif not Member['RemarkName'].encode('utf-8') in txtContact:  # RemarkName不在txt里
            MemberList.remove(Member)
    return MemberList
...
def main():
    ...
    # todo 删除群组
    webwxsendmsg(u'欢迎进群',ChatRoomName)
    print('\n---------------------------------------------')
    print('\n添加完毕...')



def webwxsendmsg(word,to):
    print("webwxsendmsg")
    url = base_uri + \
...
def _transencoding(data):
    return data.encode('utf8')

#读过滤文件，只有过滤文件中出现的注释才会被加到群里，否则不加群
def readFile():

    f = codecs.open('list.txt','r','gbk')
    s = f.readlines()
    f.close()
    for line in s:
        txtContact.append(line.encode('utf-8').strip('\r\n'))
